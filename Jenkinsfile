pipeline {
    ...
    environment {
        ...
        EC2_HOST = "3.108.54.51"      // Replace with your EC2 instance IP
    ...
    steps {
        script {
            ...
            sh '''
            scp -o StrictHostKeyChecking=no -i /var/lib/jenkins/appkey.pem -r app/main.py app/templates ubuntu@3.108.54.51:/home/ubuntu/ec2-jenkins/app/
            '''
            
            echo "Restarting application on EC2..."
            sh '''
            ssh -o StrictHostKeyChecking=no -i /var/lib/jenkins/appkey.pem ubuntu@3.108.54.51<<EOF
